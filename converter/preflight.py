...
def run_preflight(
    ...
    for sub_name, members in sub_rules.items():
        ...
        for index, member in enumerate(members):
            ...
            if kind == "MATCH":
                ...
                pass
            # Also check Egern sub-rule parsing compatibility
    ...
    # 4. Check Segment Metadata & Mapping
    if segment_names_path is not None:
        ...
        # 5. Check Reject Segments Policy & Contract
        ...
        for index, raw in enumerate(rules):
            ...
            if seg_name in expected_reject_policies:
                ...
                if ref is not None:
                    ...
                    # Check if policy or sub-rule policy matches expected reject policy
                    if ref.wrapper_kind == "SUB-RULE":
                        # If wrapped in sub-rule, check sub-rule terminal match or members
        ...
        # 6. Check Export Config Profile if present
        if export_config_path is not None and export_config_path.exists():
            try:
            ...
            # Check DNS Groups vs Segment Roles
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("input", type=Path, help="Path to rules.yaml")
